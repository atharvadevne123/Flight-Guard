...
CARRIER_WEATHER_SENSITIVITY = {
    "AA": 1.10, "DL": 0.95, "UA": 1.05, "WN": 1.15, "B6": 1.20,
    "AS": 1.00, "NK": 1.25, "F9": 1.22, "G4": 1.18, "SY": 1.12,
}
    ...
    def score_carrier(
        ...
        if history_df is not None and not history_df.empty:
            ...
            if sample_size > 0 and "dep_delay_minutes" in carrier_hist.columns:
                ...
                mean_delay = float(
                    carrier_hist.loc[delayed, "dep_delay_minutes"].mean()
                    if delayed.any() else 0.0
                )
                p90_delay = float(
    ...
    def score_route(
        ...
        if history_df is not None and not history_df.empty:
            ...
            if sample_size > 0 and "dep_delay_minutes" in route_hist.columns:
                ...
                mean_delay = float(
                    route_hist.loc[delayed, "dep_delay_minutes"].mean()
                    if delayed.any() else 0.0
                )
                p90_delay = float(
